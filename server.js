...
app.use((req, res, next) => {
  ...
  res.redirect = function(url) {
    if (req.headers.accept && req.headers.accept.includes('application/json')) {
      ...
      const errorMsg = messages.error ? messages.error[0] : null;
      const successMsg = messages.success ? messages.success[0] : null;
      
      const success = !errorMsg;
      const message = errorMsg || successMsg || 'Success';
      
      return res.json({ success, message, redirect: url });
    }
  ...
  res.render = function(view, locals, cb) {
    if (req.headers.accept && req.headers.accept.includes('application/json')) {
      ...
      const success = !errorMsg;
      const message = errorMsg || successMsg || 'Loaded';
